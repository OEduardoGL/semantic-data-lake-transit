#!/usr/bin/env python3
"""Etapa 7 (Bloco D): executa Prompt 1 contra uma LLM e salva artefatos.

Este script suporta o modelo open source de comparacao primeiro:
- provider: openrouter
- model: meta-llama/llama-3.3-70b-instruct

Tambem pode ser reutilizado depois para OpenAI.
"""
...
def load_local_env(base_dir: Path) -> None:
    """Carrega variaveis simples de tcc/.env.local sem dependencias extras."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Executa prompt de wrangling contra uma LLM")
...
def resolve_prompt_file(base_dir: Path, explicit_path: str | None) -> Path:
    ...
    candidates = sorted(artifacts_dir.glob("prompt_1_wrangling_rendered_*.md"))
    if not candidates:
        raise FileNotFoundError("Nenhum prompt renderizado encontrado em tcc/artifacts/")
    return candidates[-1]
...
def build_output_paths(base_dir: Path, provider: str, model: str) -> tuple[Path, Path]:
    ...
    code_path = generated_dir / f"wrangling_{slug}_{ts}.py"
    log_path = artifacts_dir / f"wrangling_call_{slug}_{ts}.json"
...
def main() -> None:
        ...
        messages=[
            {
                "role": "system",
                "content": (
                    "You generate complete Python code for PySpark data wrangling. "
                    "Return only executable code, preferably in a single python code block."
                ),
            },
    ...
    print("=== Wrangling Code Generated ===")
    print(f"provider: {provider}")
    print(f"model: {model}")
    print(f"prompt_file: {prompt_path}")
    print(f"code_output: {code_path}")
    print(f"log_output: {log_path}")
    print("\n[OK] Codigo de wrangling gerado e salvo.")

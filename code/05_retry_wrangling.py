#!/usr/bin/env python3
"""Etapa 8 (Bloco B): executa retry de wrangling orientado por erro real."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Executa retry de wrangling contra uma LLM")
    parser.add_argument("--config", default=str(base / "config.yaml"))
    parser.add_argument(
        "--prompt-template",
        default=str(base.parent / "prompts" / "prompt_3_retry_wrangling_2026-04-03_v1.md"),
    )
    parser.add_argument(
        "--provider",
        default="primary",
    ...
    parser.add_argument("--error-file", required=True, help="Arquivo .txt com erro/traceback")
...
def build_output_paths(base_dir: Path, provider: str, model: str, attempt: int) -> tuple[Path, Path, Path]:
    ...
    prompt_path = artifacts_dir / f"prompt_3_retry_rendered_{slug}_attempt{attempt}_{ts}.md"
    code_path = generated_dir / f"wrangling_retry_{slug}_attempt{attempt}_{ts}.py"
    log_path = artifacts_dir / f"wrangling_retry_call_{slug}_attempt{attempt}_{ts}.json"
...
def render_retry_prompt(
    ...
    sections = [
        ...
        "## Tarefa",
        (
            "Corrija o codigo acima para que ele seja importavel como modulo e executavel em PySpark "
            "no contexto descrito, respeitando todas as restricoes semanticas e tecnicas."
        ),
        "",
    ]
    return "\n".join(sections)
...
def main() -> None:
    ...
    if args.dry_run:
        ...
        print("\n[OK] Dry-run concluido: script pronto para retry real da LLM.")
        return
        ...
        messages=[
            {
                "role": "system",
                "content": (
                    "You correct previously generated PySpark wrangling code. "
    ...
    print("=== Retry Wrangling Code Generated ===")
    print(f"provider: {provider}")
    print(f"model: {model}")
    print(f"attempt: {args.attempt}")
    print(f"prompt_rendered: {prompt_path}")
    print(f"code_output: {code_path}")
    print(f"log_output: {log_path}")
    print("\n[OK] Codigo corrigido gerado e salvo.")

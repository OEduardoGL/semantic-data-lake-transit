...
def run_ml_pipeline(df_final):
    ...
    df_final[num_cols] = df_final[num_cols].fillna(df_final[num_cols].mean())
    ...
    df_final['cod_linha'] = le.fit_transform(df_final['cod_linha'])
    df_final['cod_veiculo'] = le.fit_transform(df_final['cod_veiculo'])
    ...
    predictions_df = pd.DataFrame({
        'event_dt': df_final['event_dt'].iloc[-len(y_test):],
        'janela_5min': df_final['janela_5min'].iloc[-len(y_test):],
        'cod_linha': df_final['cod_linha'].iloc[-len(y_test):],
        'cod_veiculo': df_final['cod_veiculo'].iloc[-len(y_test):],
        'y_true': y_test,
        'y_pred': y_pred
    })

...
def run_ml_pipeline(df_final):
    ...
    line_hour_metrics_df = predictions_df.groupby(["cod_linha", pd.Grouper(key="janela_5min", freq="H")]).agg({
        "y_pred": "mean",
        "y_true": "mean",
    }).reset_index()
    ...
    line_hour_metrics_df['n_registros'] = predictions_df.groupby(["cod_linha", pd.Grouper(key="janela_5min", freq="H")]).size().values
    ...
    vehicle_day_metrics_df['n_registros'] = predictions_df.groupby(["event_dt", "cod_veiculo"]).size().values

...
from pyspark.sql.functions import col, when, expr, window, lit, from_unixtime
...
def build_final_table(df_dict: dict[str, DataFrame]) -> DataFrame:
    ...
    # Convert event_ts from BIGINT to TIMESTAMP
    combined_df = combined_df.withColumn("event_ts", from_unixtime(col("event_ts")))

    # Create final table
    final_table = combined_df.withColumn("janela_5min", window(col("event_ts"), "5 minutes").start) \

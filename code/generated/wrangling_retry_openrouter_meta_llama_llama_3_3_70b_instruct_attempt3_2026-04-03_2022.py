...
def build_final_table(df_dict: dict[str, DataFrame]) -> DataFrame:
    ...
    final_table = final_table.select(gps_events["event_dt"].alias("event_dt"), 
                                      gps_events["cod_linha"].alias("cod_linha"), 
                                      gps_events["cod_veiculo"].alias("cod_veiculo"), 
                                      F.window(gps_events["event_ts"], "5 minutes").alias("janela_5min"), 
                                      F.lag(gps_events["event_ts"], 1).over(Window.orderBy(gps_events["event_ts"]).rowsBetween(-1, 1)).alias("headway_observado_min"), 
                                      F.lit(8).alias("headway_referencia_min"), 
                                      (gps_events["event_ts"] - F.lag(gps_events["event_ts"], 1).over(Window.orderBy(gps_events["event_ts"]).rowsBetween(-1, 1))).alias("atraso_aproximado_min"))
...
if __name__ == "__main__":
    ...
    spark = SparkSession.builder.appName("Wrangling").getOrCreate()
